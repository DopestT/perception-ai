...
const leadEndpoint = 'https://zxmdfmiueapjhktqchts.supabase.co/functions/v1/microsite-lead';
...
function form() {
  return `<form class="lead-form" id="lead-form">
    <div class="grid"><label>Name<input name="name" autocomplete="name" maxlength="120" required></label><label>Phone<input name="phone" type="tel" autocomplete="tel" maxlength="40" required></label></div>
    <div class="grid"><label>Email<input name="email" type="email" autocomplete="email" maxlength="160"></label><label>ZIP code<input name="zip" inputmode="numeric" autocomplete="postal-code" maxlength="10" required></label></div>
    <label>What are you seeing?<textarea name="issue" rows="5" maxlength="1500" placeholder="Water after heavy rain, damp crawl space, sump pump issue..." required></textarea></label>
    <label class="hp" aria-hidden="true">Website<input name="website" tabindex="-1" autocomplete="off"></label>
    <input type="hidden" name="site_domain" value="hagerstownbasementwaterproofing.com">
  ...
  (() => {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (!response.ok || !payload.ok) throw new Error(payload.error || 'unavailable');
        form.reset();
        status.textContent = 'Request received. We’ll use these details to identify an appropriate local provider when available.';
      } catch (error) {
        status.textContent = error && error.message === 'rate_limited'
