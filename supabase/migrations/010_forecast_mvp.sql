...
-- This migration is additive: it does not replace existing hardened Project World functions.
...
create table if not exists public.perception_forecasts (
  ...
  original_probability double precision not null check (original_probability between 0 and 1),
  current_probability double precision not null check (current_probability between 0 and 1),
  ...
  outcome boolean,
  brier_score double precision check (brier_score is null or brier_score between 0 and 1),
...
create index if not exists perception_forecasts_user_status_idx
  on public.perception_forecasts(user_id, status, updated_at desc);
create index if not exists perception_forecasts_project_idx
  on public.perception_forecasts(project_id, created_at desc);
create index if not exists perception_forecast_versions_forecast_idx
  on public.perception_forecast_versions(forecast_id, version desc);
create index if not exists perception_forecast_versions_user_idx
  on public.perception_forecast_versions(user_id, created_at desc);
...
begin
  ...
  insert into public.perception_forecasts (
    user_id, project_id, question, deadline,
    original_probability, current_probability, confidence, trend, model_breakdown
  ) values (
    v_user, v_project_id, v_question, p_deadline,
    v_probability, v_probability, 'low', 'steady',
  ...
  return (
    select jsonb_build_object('ok', true, 'project_id', f.project_id, 'forecast', to_jsonb(f), 'version', 1)
    from public.perception_forecasts f where f.id = v_forecast_id
  );
end;
$$;
...
begin
  ...
  v_trend := case
    when p_probability > v_forecast.current_probability then 'up'
  ...
  return (
    ...
    from public.perception_forecasts f where f.id = p_forecast_id
  );
end;
$$;
...
begin
  ...
  values
    (
      v_user, v_forecast.project_id, 'forecast.resolved',
      jsonb_build_object(
        'forecast_id', p_forecast_id,
        'outcome', p_outcome,
        'final_probability', v_forecast.current_probability,
        'brier_score', v_score
      )
    ),
    (
      v_user, v_forecast.project_id, 'calibration.updated',
      jsonb_build_object('forecast_id', p_forecast_id, 'brier_score', v_score)
    );
