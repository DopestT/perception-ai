...
>
> **Current execution blocker is explicit, not hidden:** the active primary Project World source for `DopestT/perception-ai` identifies the repository but currently does not carry an observed default branch, and a natural-language objective does not yet yield an exact file payload. The resolver therefore stops at `needs_input` instead of inventing either value. The next runtime slice should refresh repository observations with default-branch/head evidence and connect a bounded file-change generator/artifact into the resolver, then transition a complete contract to `awaiting_permission` when no scoped grant exists.
>
